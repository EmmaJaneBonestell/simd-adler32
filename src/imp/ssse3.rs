...
mod imp {
    ...
    use core::arch::x86::{
        ...
        _mm_maddubs_epi16, _mm_sad_epu8, _mm_set_epi8, _mm_set_epi32, _mm_set1_epi16,
        _mm_shuffle_epi32, _mm_slli_epi32, _mm_unpackhi_epi64,
    };
    ...
    unsafe fn reduce_add_blocks<'a>(a: &mut u32, b: &mut u32, chunk: &'a [u8]) -> &'a [u8] {
        unsafe {
            ...
            for block in blocks {
                ...
                let mad1 = _mm_maddubs_epi16(right_v, weight_lo_v);
                b_v = _mm_add_epi32(b_v, _mm_madd_epi16(mad1, one_v));
            }
...
fn get_imp_inner() -> Option<Adler32Imp> {
    std::arch::is_x86_feature_detected!("ssse3").then_some(imp::update as Adler32Imp)
}
